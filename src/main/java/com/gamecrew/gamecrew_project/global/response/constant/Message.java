package com.gamecrew.gamecrew_project.global.response.constant;

public class Message {
    public static final String SIGNUP_SUCCESSFUL = "회원가입을 완료하였습니다.";
    public static final String AUTH_SUCCESSFUL = "이메일 인증에 성공하였습니다.";
    public static final String AVAILABLE_NICKNAME = "사용 가능한 닉네임입니다.";
    public static final String NICKNAME_UPDATE_SUCCESSFUL = "닉네임 수정 완료";
    public static final String AVAILABLE_EMAIL = "사용 가능한 이메일입니다.";
    public static final String CREATE_CHAT_ROOM = "채팅방을 생성하였습니다.";
    public static final String SEARCH_CHAT_ROOM = "채팅방 조회에 성공했습니다.";
    public static final String POST_SUCCESSFUL = "등록 완료";
    public static final String POST_PUT_SUCCESSFUL = "수정 완료";
    public static final String POST_DELETE_SUCCESSFUL = "삭제 완료";
    public static final String REGISTRATION_COMPLETED = "점수가 정상적으로 반영되었습니다.";
    public static final String PASSWORD_UPDATE_SUCCESSFUL = "비밀번호 수정 완료";
    public static final String PASSWORD_MATCH_SUCCESSFUL = "비밀번호가 일치합니다.";
}
